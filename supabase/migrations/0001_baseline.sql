...
CREATE OR REPLACE FUNCTION public.claim_signup_slot(p_bucket text, p_limit integer)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  v_n int;
begin
  -- Opportunistic cleanup. Signup is rare enough that paying for this on every
  -- call is cheaper than owning a scheduled job, and it keeps the table from
  -- being an indefinite (if opaque) record of who visited.
  delete from signup_attempts where created_at < now() - interval '1 day';

  insert into signup_attempts (bucket, n)
  values (p_bucket, 1)
  on conflict (bucket) do update
    set n = signup_attempts.n + 1
...
CREATE TABLE public.class_material (
  ...
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  modes text[] DEFAULT '{spelling,sight-words}'::text[] NOT NULL,
...
CREATE TABLE public.families (
  ...
  tier text DEFAULT 'full'::text NOT NULL
);
...
CREATE TABLE public.kid_deep_discoveries (
  kid_id uuid NOT NULL,
  family_id uuid NOT NULL,
  landmark_slug text NOT NULL,
  found_at timestamp with time zone DEFAULT now() NOT NULL
);
CREATE TABLE public.kid_deep_state (
  kid_id uuid NOT NULL,
  family_id uuid NOT NULL,
  x double precision DEFAULT 0 NOT NULL,
  y double precision DEFAULT '-14'::integer NOT NULL,
  z double precision DEFAULT '-420'::integer NOT NULL,
  heading double precision DEFAULT 0 NOT NULL,
  deepest_m double precision DEFAULT 0 NOT NULL,
  dives integer DEFAULT 0 NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
);
...
ALTER TABLE public.kid_deep_discoveries ADD CONSTRAINT kid_deep_discoveries_pkey PRIMARY KEY (kid_id, landmark_slug);
ALTER TABLE public.kid_deep_state ADD CONSTRAINT kid_deep_state_pkey PRIMARY KEY (kid_id);
ALTER TABLE public.kid_game_unlocks ADD CONSTRAINT kid_game_unlocks_pkey PRIMARY KEY (kid_id, game_slug);
ALTER TABLE public.kid_items ADD CONSTRAINT kid_items_pkey PRIMARY KEY (kid_id, item_slug);
...
ALTER TABLE public.signup_attempts ADD CONSTRAINT signup_attempts_pkey PRIMARY KEY (bucket);
...
ALTER TABLE public.invite_codes ADD CONSTRAINT invite_codes_claim_family_id_fkey FOREIGN KEY (claim_family_id) REFERENCES families(id) ON DELETE SET NULL;
...
ALTER TABLE public.kid_deep_discoveries ADD CONSTRAINT kid_deep_discoveries_family_id_fkey FOREIGN KEY (family_id) REFERENCES families(id) ON DELETE CASCADE;
ALTER TABLE public.kid_deep_discoveries ADD CONSTRAINT kid_deep_discoveries_kid_id_fkey FOREIGN KEY (kid_id) REFERENCES kids(id) ON DELETE CASCADE;
ALTER TABLE public.kid_deep_state ADD CONSTRAINT kid_deep_state_family_id_fkey FOREIGN KEY (family_id) REFERENCES families(id) ON DELETE CASCADE;
ALTER TABLE public.kid_deep_state ADD CONSTRAINT kid_deep_state_kid_id_fkey FOREIGN KEY (kid_id) REFERENCES kids(id) ON DELETE CASCADE;
...
ALTER TABLE public.kid_items ADD CONSTRAINT kid_items_family_id_fkey FOREIGN KEY (family_id) REFERENCES families(id) ON DELETE CASCADE;
ALTER TABLE public.kid_items ADD CONSTRAINT kid_items_kid_id_fkey FOREIGN KEY (kid_id) REFERENCES kids(id) ON DELETE CASCADE;
...
ALTER TABLE public.class_material ADD CONSTRAINT class_material_modes_not_empty CHECK (((kind <> 'words'::text) OR (array_length(modes, 1) >= 1)));
...
ALTER TABLE public.families ADD CONSTRAINT families_tier_valid CHECK ((tier = ANY (ARRAY['full'::text, 'restricted'::text])));
...
ALTER TABLE public.kid_deep_discoveries ADD CONSTRAINT kid_deep_discoveries_landmark_slug_check CHECK ((length(TRIM(BOTH FROM landmark_slug)) > 0));
ALTER TABLE public.kid_deep_state ADD CONSTRAINT kid_deep_state_deepest_m_check CHECK ((deepest_m >= (0)::double precision));
ALTER TABLE public.kid_deep_state ADD CONSTRAINT kid_deep_state_dives_check CHECK ((dives >= 0));
ALTER TABLE public.kid_game_unlocks ADD CONSTRAINT kid_game_unlocks_cost_paid_check CHECK ((cost_paid >= 0));
ALTER TABLE public.kid_items ADD CONSTRAINT kid_items_item_slug_check CHECK ((length(TRIM(BOTH FROM item_slug)) > 0));
ALTER TABLE public.kid_items ADD CONSTRAINT kid_items_source_check CHECK ((length(TRIM(BOTH FROM source)) > 0));
...
CREATE INDEX families_tier_idx ON public.families USING btree (tier);
...
CREATE INDEX kid_deep_discoveries_kid_idx ON public.kid_deep_discoveries USING btree (kid_id);
...
CREATE INDEX kid_items_kid_idx ON public.kid_items USING btree (kid_id);
...
CREATE INDEX signup_attempts_created_at_idx ON public.signup_attempts USING btree (created_at);
...
CREATE UNIQUE INDEX token_transactions_milestone_idempotent_idx ON public.token_transactions USING btree (kid_id, ((metadata ->> 'milestone_id'::text))) WHERE (metadata ? 'milestone_id'::text);
...
ALTER TABLE public.kid_deep_discoveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.kid_deep_state ENABLE ROW LEVEL SECURITY;
...
-- -------------------------------------------------------------- 11. seed data
-- Skills catalog: 68 rows. Universal content, no family_id and
...
INSERT INTO public.skills ("id", "subject", "name", "display_name", "tier", "standard_code", "standard_desc", "grade_level", "on_track_tier", "domain", "gamifiable") VALUES
  ...
  ('10000000-0000-4000-8000-000000000063', 'math', 'rounding', 'Rounding', 6, '3.NBT.A.1, 4.NBT.A.3', 'Use place value understanding to round whole numbers to the nearest 10 or 100 (3rd); round multi-digit numbers to any place (4th)', '3-4', 7, 'place-value', true),
  ...
  ('20000000-0000-4000-8000-000000000056', 'reading', 'word-meaning', 'Word meanings', 4, 'L.1.6, L.2.6, L.3.6', 'Acquire and use grade-appropriate words and phrases taught directly or through reading', '1-3', 4, 'vocabulary', true),
  ...
  ('30000000-0000-4000-8000-000000000003', 'logic', 'sudoku', 'Sudoku', 1, NULL, NULL, NULL, NULL, NULL, true),
  ('30000000-0000-4000-8000-000000000004', 'logic', 'hanoi', 'Cake Shift', 1, NULL, NULL, NULL, NULL, NULL, true)
ON CONFLICT (id) DO NOTHING;
