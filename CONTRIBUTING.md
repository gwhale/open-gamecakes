...
3. **Regenerate the baseline in the SAME pull request.** A migration and the
   baseline are one change, not two.
   ...
   node scripts/opensource/baseline.mjs generate <project-ref>      > supabase/baseline/0001_baseline.sql
   ```

   `supabase/baseline/0001_baseline.sql` is the **entire schema** for anyone who
   is not the founding deployment: `publish.mjs` deletes `supabase/migrations/`
   from the public cut and ships that one file in its place.

   So a migration without a regenerated baseline is invisible on your machine
   and broken on everyone else's. It has already happened once — 0048 and 0049
   added two columns the code selects, the baseline went out without them, and
   a fresh install 500'd on the parent skills tab. Nobody here could have
   noticed, because here the columns exist.

   `npm run check:baseline` enforces this, and CI runs it. If your migration
   genuinely changes no schema, regenerate anyway: the diff will be empty and
   the check passes.
