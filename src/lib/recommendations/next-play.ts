...
const SKILL_TO_PRACTICE: Record<string, { game: string; op?: string }> = {
  ...
  // Same op, different game, deliberately. The two place-value rows are the
  // ones most likely to be due at the same time for the same kid, and the
  // de-dupe below drops the second pick that lands on a game already taken —
  // so sharing cakey-tower would mean only ever seeing one of them.
  'place-value-thousands':   { game: '/games/sandcastle-siege', op: 'place-value' },
  'skip-counting':           { game: '/games/pacman-cakey',    op: 'skip-count' },
  'rounding':                { game: '/games/cakey-road',      op: 'rounding' },
...
/**
 * Which KIND of question a skill is practised with — the `op` half of
 * practiceHref, without the game half.
 *
 * For math this is a MathType or ConceptType; for reading it is a
 * ReadingChallengeType. Both are exactly what generateChallengeForMode wants,
 * which is why this table can drive a question directly and not only a link.
 *
 * Exported for the treasure-chest gate (src/lib/deep/gate.ts), which asks a kid
 * about the skill the recommender picked but has no game to send them to — the
 * question happens where they are standing, on the seabed.
 */
export function practiceOpFor(skillSlug: string): string | null {
  return SKILL_TO_PRACTICE[skillSlug]?.op ?? null;
}
