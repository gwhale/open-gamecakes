...
import { CAKE_SHIFT_GLYPH, CAKE_SHIFT_NAME, HANOI_SLUG } from './hanoi/name';
...
export interface GameInfo {
   ...
   *
   *  Set on every game whose shell calls verbalSkillFor(). Without it the Word
   *  Games section showed a single game while the rest quietly taught reading
   *  under a Math heading.
   *
   *  Deliberately no count here. This comment used to say "the 16 games", went
   *  stale, and got copied onto the public /open page as a fact — a number
   *  sitting next to the data it counts will always rot faster than the data.
   *  Count it: `grep -c "wordsMode: true"` across this file AND
   *  registry.local.ts. */
  wordsMode?: boolean;
...
export const UPSTREAM_GAMES: readonly GameInfo[] = [
  ...
  // Puzzle Island's first booth. FREE, as every island booth is: the ferry
  // already charges FARE + LAND.ISLAND for the trip, and the island shipped
  // scenic on the promise that its games come with it.
  //
  // Gets its OWN logic/sudoku skill (migration 0057), for checkers' reason:
  // the launcher seeds difficulty from kid_skills.current_tier on the skill
  // it reports to, and a waffle tier must never decide which Cakey a kid
  // faces at chess — nor a chess tier how sparse their first waffle is.
  // No wordsMode: there are no questions in it at all.
  { slug: 'sudoku',          label: 'Waffle Sudoku',    glyph: '🧇', subject: 'logic' },
  // Puzzle Island — Tower of Hanoi on three cake stands, in three.js. FREE,
  // like every island booth. Slug + skill are `hanoi`; the kid-facing name
  // and glyph come from ONE constant so George can rename it in one place.
  // (Sudoku's row, from the sibling PR, belongs directly above this one.)
  //
  // ⚠️ Glyph is 🗼, not 🥞: the pancake stack is the better fiction, but it
  // would sit next to the 🧇 Sudoku booth as two golden-brown breakfasts on
  // one booth row — the same two-similar-glyphs seam the crane comment above
  // warns about. 🎂/🍰/🍡/🧁 are all taken. Non-edible glyphs have precedent.
  { slug: HANOI_SLUG,        label: CAKE_SHIFT_NAME,    glyph: CAKE_SHIFT_GLYPH, subject: 'logic' },
  // Vocab Land
  ...
  // Puzzle Island's word booths. Both are READING games natively — a word
  // search of sight words and a hear-it-spell-it candle game — so they carry
  // `subject: 'reading'` and NOT `wordsMode`: that flag marks a math game
  // whose launcher offers a Words toggle, and there is no math here to
  // toggle away from. Each credits an existing reading skill through
  // verbalSkillFor() (sight-words-<grade>, spelling-patterns); no new row.
  //
  // FREE, like every island booth: the island already cost FARE + LAND.ISLAND
  // to reach (see chess-challenge above for the argument).
  //
  // Both draw from the kid's class word list first when a grown-up has added
  // one — see lib/games/words/pool.ts. That is the reason they exist.
  { slug: 'word-search',     label: 'Sprinkle Search',  glyph: '🔍', subject: 'reading' },
  // The slug is what grown-ups call this game; nothing a kid reads says it.
  // On screen it is "Cakey's Candles" (one name everywhere): a cake whose candles
  // blow out one per wrong letter. No gallows. 🎧 because the word is an
  // audio clue — Cakey says it, the kid never sees it until it is spelled.
  { slug: 'hangman',         label: "Cakey's Candles", glyph: '🎧', subject: 'reading' },
];
