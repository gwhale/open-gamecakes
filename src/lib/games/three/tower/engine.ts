...
//   • GOOD  — mint cherry-top petit four (no face): tap to EAT. Eat every one
//             to WIN. One that falls off the plate is gone — no longer needed.
//   • BAD   — strawberry gummy CREATURE (candy eyes): must NOT tumble off the
//             plate and splat — each splat costs a life. The only way to lose.
//   • HARD  — matte chocolate-brittle: heavy, can't be eaten, but it DOES fall
//             and knock things. (It used to be mass 0 and would hang in mid-air
//             once everything under it was eaten.)
//   • MYSTERY — purple gobstopper: tap it and it is either a mint eaten on the
//             spot (lucky!) or a new gummy standing where it was.
//
// What each outcome MEANS is decided in rules.ts (pure, tested); this file
// only detects it — a raycast hit, a body below the plate — and draws it.
//
...
const DANGER_Y = PLATE_Y - 0.4; // a block whose centre drops below this has left the plate
const HARD_MASS = 4;           // chocolate brittle: ~4 mints' worth — it falls, and it shoves
...
export interface CakeyTowerCallbacks {
  ...
  /** Something a kid needs told in words, because it happened a second after
   *  their last tap and the physics did it, not them:
   *    'splat'        — a gummy fell off the plate, −1 life
   *    'mint-tumbled' — a mint fell off; it is gone and no longer needed
   *    'lucky'        — the gobstopper was a mint, eaten on the spot
   *    'gummy'        — the gobstopper hardened into a gummy; keep it on the plate */
  onNotice?(kind: 'splat' | 'mint-tumbled' | 'lucky' | 'gummy'): void;
}
...
export function createCakeyTowerEngine(
    ...
    for (let c = 0; c < tuning.courses; c++) {
      ...
      for (let ix = 0; ix < TOWER_W; ix++) {
        for (let iz = 0; iz < TOWER_D; iz++) {
          const role = rollRole(c, tuning, Math.random);
          ...
          g.rotation.y = yaw;
          scene.add(g);
          // Brittle is HEAVY, not static. A mass-0 body never moves, so once
          // everything under it was eaten it hung in the air — a kid sees a
          // floating chocolate bar and stops trusting the physics. Heavy means
          // it drops when unsupported and shoves whatever it lands on, which
          // is what "it's holding things up" should have meant all along.
          const body = new CANNON.Body({
            mass: role === 'hard' ? HARD_MASS : 1,
          ...
          body.sleep(); // the whole stack starts at rest; the first bite wakes what it touches
          world.addBody(body);
  ...
  // The two numbers the win and the score screen come from. Only rules.ts
  // moves them — the engine never does `goodLeft--` by hand.
  let tally: CandyTally = { goodTotal, goodLeft: goodTotal };
  ...
  cb.onCandiesLeft(tally.goodLeft, tally.goodTotal);
  ...
  const setTally = (t: CandyTally): void => {
    tally = t;
    cb.onCandiesLeft(tally.goodLeft, tally.goodTotal);
    if (isCleared(tally)) endRound(true);
  };
  ...
  // Anything that is not a gummy and leaves the plate is simply gone: a puff,
  // no heart lost. A mint that goes this way stops counting toward the win —
  // before this, a mint knocked onto the table was still "left to eat", there
  // was no way to reach it, and there is no timer, so the round never ended.
  const tumbleBlock = (blk: Block): void => {
    const at = new THREE.Vector3(blk.body.position.x, blk.body.position.y, blk.body.position.z);
    debris.push(spawnDustPuff(THREE, scene, at));
    removeBlock(blk);
    if (blk.role === 'good') {
      cb.onNotice?.('mint-tumbled');
      setTally(applyTumble(tally, blk.role));
    }
  };
  ...
  const onPointerDown = (e: PointerEvent): void => {
    ...
    if (blk.role === 'good') {
      ...
      // Gamble: a mint eaten on the spot, or a new gummy. Consumes the bite.
      bites--;
      cb.onBitesLeft(bites);
      resolveMystery(blk, at);
      if (!ended) scheduleResolve();
    } else {
      // bad or hard — can't be eaten. No bite spent; gentle "nope".
      cb.onSfx?.('nope');
    }
  };
  renderer.domElement.addEventListener('pointerdown', onPointerDown);

  const resolveMystery = (blk: Block, at: THREE.Vector3): void => {
    const outcome = flipMystery(Math.random);
    if (outcome === 'lucky') {
      // It was a mint all along — and the bite is already in your mouth. Eat
      // it now rather than revealing a live mint: a revealed mint RAISED the
      // number left to eat, so a correct answer could make the round longer.
      cb.onNotice?.('lucky');
      cb.onSfx?.('eat');
      debris.push(spawnDebris(THREE, scene, at, 10, 0xa855f7, [0x86efac, 0xffffff, 0xfde68a], 0.7));
      debris.push(spawnDustPuff(THREE, scene, at));
      removeBlock(blk);
      setTally(applyMysteryOutcome(tally, outcome));
      return;
    }
    // Hardens into a gummy where it stands: swap the mesh contents in place.
    cb.onNotice?.('gummy');
  ...
  const tick = (): void => {
    ...
    if (!paused && !ended) {
      ...
      for (const blk of blocks) {
        ...
        syncMesh(blk);
        // Off the plate: a gummy splats (−1 life); anything else tumbles away.
        if (fellOffPlate(blk.body.position.y, DANGER_Y)) {
          if (dropOutcome(blk.role) === 'splat') splatBlock(blk);
          else tumbleBlock(blk);
        }
      }
    }
  ...
  return {
    ...
    getStats() { return { goodLeft: tally.goodLeft, goodTotal: tally.goodTotal, lives, startLives, won }; },
    dispose(): void {
      ended = true;
