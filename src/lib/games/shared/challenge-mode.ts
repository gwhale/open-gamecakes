...
export function mathSkillFor(
  ...
  switch (mathType) {
    ...
    case 'place-value':
      // Two rows, one picker entry, split where the generator splits. Below
      // tier 5 the question is tens, ones and hundreds — 1.NBT.B.2, a first
      // grade standard. At tier 5 the four-digit band opens (expanded form,
      // naming a place in 3,486, stepping by a power of ten), which is
      // 2.NBT.A.1 / 2.NBT.A.3 / 3.NBT.A.2 and has its own catalog row.
      // Crediting all of it to `place-value` showed a third grader working
      // through their own chapter as doing grade-1 work.
      //
      // It does NOT climb to place-value-millions at tier 8: the generator
      // stops at 9,999, and a row that says "place value to millions" has to
      // mean the kid saw one.
      return slug(t <= 4 ? 'place-value' : 'place-value-thousands');
    case 'skip-count':
      return slug('skip-counting');
    case 'rounding':
      return slug('rounding');
    case 'shapes':
