 ...
 *
 *  `from` defaults to the singleton. A launcher that wants to SAY whether the
 *  kid's list will be used has not called setClassLists() yet (that happens on
 *  Play), so it passes the lists it is holding and gets the same answer the
 *  game will. */
export function poolForMode(
  mode: ClassWordMode,
  from: readonly ClassWordList[] = lists,
): {
  words: string[];
  glosses: Record<string, string>;
} {
