...
/** What the guest sandbox starts the session with.
 *
 *  NOT a price and not a balance a real kid can reach — a demo allowance. The
 *  sandbox exists to SHOW the game, and a demo that runs out of money stops
 *  demonstrating the things most worth showing. Sits far above the dearest
 *  thing on the ladder (ESTATE.CASTLE) so nothing in the sandbox is ever
 *  refused for want of coins.
 *
 *  It is spent against nothing: the guest has no wallet row and every sink in
 *  the town already waives its charge for guests (rides, storms, game unlocks,
 *  land reveals). This is the number on the pill, so the HUD reads like a kid's
 *  who has been playing a while rather than like an empty account. */
export const GUEST_BALANCE = 999;
