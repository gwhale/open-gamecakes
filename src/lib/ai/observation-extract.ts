...
/** Skill slugs the extractor may suggest. Every one must be a real row in
 *  `skills` — mostly 0003_seed_skills.sql, plus the later seeds noted below.
 *  Kept in sync manually.
 *
 *  This list is what the extractor can say about a photographed worksheet, so
 *  a whole chapter missing from it is a chapter that comes back tagged as
 *  something else. That is what happened to a grade-3 "Numbers to 10,000"
 *  chapter test: every rounding item on it had to land on `skip-counting` or
 *  nothing. */
const KNOWN_SKILL_SLUGS = [
  ...
  'multiply-within-25',
  'place-value',
  // 0007_k6_competency_framework.sql
  'place-value-thousands',
  // 0055_seed_rounding_skill.sql
  'rounding',
  // reading
  'letter-sounds',
...
export async function extractObservationFromPhoto(args: {
  ...
  const apiKey = aiKey();
  if (!apiKey) {
    return { ok: false, reason: aiUnavailableReason() };
  }
