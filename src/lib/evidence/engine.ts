...
export async function runEvidenceEngine(
  ...
  // --- 1. Load kid metadata (name + grade + family) ---
  const { data: kidRow, error: kidErr } = await sb
    .from('kids')
    .select('id, name, grade, grade_year, family_id')
    .eq('id', input.kidId)
    .maybeSingle();
  if (kidErr || !kidRow) {
  ...
  const kidGrade = gradeLabel(currentGradeOf(kidRow));

  // --- 1b. THE MONEY BOUNDARY ---
  //
  // Everything past this point can reach a paid model. The check lives HERE
  // rather than on the four routes that call this function, because a fifth
  // caller is the likely future and it would not know to add one. One guard at
  // the point of spend cannot be forgotten by a caller that does not exist yet.
  //
  // /api/attempts deliberately carries the 'play' capability, not 'ai' — a
  // restricted family must be able to play, and play is what triggers this. So
  // the tier question has to be asked again, on this side.
  const { data: famRow } = await sb
    .from('families')
    .select('tier')
    .eq('id', kidRow.family_id as string)
    .maybeSingle();
  if (!can((famRow as { tier: string } | null)?.tier, 'ai')) {
    return { ok: false, reason: 'ai is not enabled for this family' };
  }
...
// kidGradeFromName lived here: it matched the founding family's two children
// by the first four letters of their names and returned '1' for everyone else,
// INSIDE the prompt sent to the model. So every other family's kid was
// evaluated as a first grader regardless of their actual grade.
//
// Its own comment said "when we add a grade column to kids, replace this" —
// that column arrived in 0015, and grade_year (0044) made it self-advancing.
// This now reads it, via the same currentGradeOf() the rest of the app uses.
//
// Worth noting for the founder-name check: it never saw this, because the
// names were lowercase string literals inside a comparison rather than prose.
