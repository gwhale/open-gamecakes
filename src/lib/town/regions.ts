...
//   col0     col1      col2      col3
// ┌────────┬─────────┬─────────┬──────┐
// │ chess  ╎  chess  │ castle ╎ puzzle│  tier 0 (y 0..2)   chess and puzzle are
// │────────┼─────────┼─────────┼──────│                    both 8×6 and spill
// │tree hs │ club hs │mountain╎ isle  │  tier 1 (y 3..5)   over the lands under
// │────────┼─────────┼─────────┼──────│                    the overlap note
...
// only. Chess, Race and Puzzle are separate islands whose real placement is
// solved at runtime by islands.ts (pushed offshore from the mainland's actual
// shoreline). They are laid out here to MATCH the solved bearing — chess NW,
// race to the south, puzzle NE — so this map still reads like the world you
// actually walk.
//
...
// Caramel Cove is DOUBLE-WIDE. It is the only land that is also a harbour --
// the jetty, the moored submarine and the whole entrance to the Sunken
// Batterlands stand on it -- and at the standard 4x3 the map gave the island's
// only sea gate the same footprint as a scenic beach.
//
// It grows WEST into free tiles rather than south into open water, and that is
// deliberate: islandExtentPx() bounds the mainland from its region rects, so a
// rect that reaches past the current bbox moves the island's centre and every
// offshore island with it. Measured, growing south by 3 tiles shifted the
// mainland 470px and moved Chess and Race Islands about 1000px each. This
// stays inside the existing bbox, so nothing else on the map moves at all.
const CV = { x: 8, y: 9 } as const;
const COVE_SIZE = { w: 8, h: 3 } as const;
...
// Puzzle Island — the top-RIGHT quarter. Offshore like Chess, so this rect is
// bookkeeping: its centre sets the bearing the solver pushes the island out
// along (NE), center(tile,size) is the spawn point, and its SIZE sets the
// plaza pad and the booth row. Nothing else reads it.
//
// 8×6 anchored at (8,0), the mirror of Chess's 8×6 at (0,0), for the same two
// reasons. It holds FOUR booths: boothOffsetsPx spreads N slots across a span
// set by the rect's width, and on a 4×3 rect four roofs overlap by ~70px and
// the tap proxies collide (booth-row.test.ts). And the centre vector from the
// pinned world centre (8,6) to (12,3) is (4,−3) — parallel to the old 4×3's
// (6,−4.5) — so the island stays on its −36.87° ray instead of swinging round
// the map. On paper it covers the castle and mountain rects; offshore rects
// are exempt from the mainland overlap rule (see the chess note above).
const PZ = { x: 8, y: 0 } as const;
const PUZZLE_SIZE = { w: 8, h: 6 } as const;
...
const UPSTREAM_REGIONS: readonly Region[] = [
  ...
  },
  {
    ...
    // NO GAMES, on purpose. This land is the harbour: the jetty, Sub-Cake One
    // and the way down to the Sunken Batterlands. It used to hold Cakey Road,
    // on the reasoning that the cove's "sticky golden waves" suited a game full
    // of syrup rivers and candy traffic — which was true of the old straight
    // beach and stopped being true the day the cove was carved into real water
    // and given a submarine. The ticket that asked for this put it plainly:
    // move the road game to the car island, and let the cove be a cove.
    //
    // Nothing requires a land to have a booth (regions.test.ts checks that every
    // GAME is placed, not that every LAND is occupied), and a land whose whole
    // point is the thing tied up at the end of its pier does not need one.
    games: [],
    spawnPoint: center(CV, COVE_SIZE),
    themeColor: '#fde68a',
    landmark: '⚓',
    ribbon: 'AMBER',
  },
  ...
  // ---- Puzzle Island ----
  // The third offshore island (see islands.ts), reached by its OWN Cakey Ferry
  // from the north-east shore. Logic games live here: no math gate, no words —
  // the puzzle IS the practice, tracked against a 'logic' skill the way Chess
  // Island's booths are. `neighbors: []` for the same reason as chess-club:
  // it makes /api/town/discover reject any walk-up server-side, so the ferry
  // (and a fly landing) are the only ways in.
  //
  // Shipped SCENIC first with games: [] the way Race Island did, so the island
  // and the ferry generalisation could be reviewed on their own; Waffle Sudoku
  // (the first booth) followed in its own PR. A second booth (a tower puzzle
  // made of cake) is planned.
  {
    slug: 'puzzle-isle',
    name: 'Puzzle Island',
    theme: 'Quiet grids and tidy rows — every square has one right answer.',
    tile: PZ,
    size: PUZZLE_SIZE,
    unlock_cost: LAND.ISLAND,
    starter: false,
    neighbors: [],
    // Booth row west→east: the two logic games, then the two word games.
    games: ['sudoku', 'hanoi', 'word-search', 'hangman'],
    spawnPoint: center(PZ, PUZZLE_SIZE),
    // Sky blue: the BLUE ribbon is the "travel" role, and the ferry's sail and
    // signpost for this island take the same colour so the whole route reads as
    // one thing. Chess owns PURPLE.
    themeColor: '#bae6fd',
    landmark: '🧩',
    ribbon: 'BLUE',
  },
  ...
  // the bridge rule is ENFORCED and not merely rendered.
  {
    ...
    neighbors: [],
    // Cakey Road moved here from Caramel Cove. Pit Row over Victory Lane on
    // theme: the hopper is about getting across busy traffic, and this is the
    // land of "tyre stacks, toolboxes, engines revving" — Victory Lane is the
    // podium at the far end, where a road full of moving cars makes no sense.
    // Two booths fit: boothOffsetsPx spreads N across the frontage, and
    // booth-row.test.ts checks they do not collide.
    games: ['pit-stop', 'cakey-road'],
