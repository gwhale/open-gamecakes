...
// channel from the mainland to ONE offshore island (Chess Island first; Puzzle
// Island second — see islands.ts `transport: 'ferry'`). The engine builds one
// of these per ferry island, so nothing in here knows an island by name: the
// far stop is whatever `FerryLayout.islandId` says, and the sail is whatever
// colour the layout hands in (each island's ribbon, so a kid can tell the boats
// apart from across the water).
//
...
// `ferrying`, the 1-token fare, and discovering the island on arrival. The
// barrier is the REAL sea (the wide islandNd channel) — no invisible walls. Like
// train.ts: no runtime `three` import (namespace passed in), gameplay math in
// city PIXELS, scene units only at mesh placement.

import type { ThreeNS } from './types';
import { pxToSceneX, pxToSceneZ } from './types';
import { CAKE, WOOD } from '@/lib/games/theme/palette';
import { cakeMat, cookieMat, glowSprite, emojiSprite } from './materials';
...
/** Which dock a stop is: the mainland, or the island id this boat serves
 *  (`FerryLayout.islandId`, e.g. 'chess-isle'). The ferry only knows these two
 *  ids; the engine maps the island id → its landing region → discovery. */
export type FerryStop = string;
export const MAINLAND_STOP: FerryStop = 'mainland';
 ...
 *  the other; the board point is on solid mainland shore; arrival is the
 *  island's dry center. Works at any island placement — no baked-in orientation. */
export interface FerryLayout {
  /** The offshore island this boat serves (islands.ts id). Doubles as the far
   *  stop id, so `depart(islandId)` sails out and `depart(MAINLAND_STOP)` home. */
  islandId: string;
  mainlandDockPx: { x: number; y: number };
  islandDockPx: { x: number; y: number };
  mainlandBoardPx: { x: number; y: number };
  arrivePx: { x: number; y: number };
  /** Sail colour — the destination's ribbon, so each boat colour-codes where it
   *  goes (Chess purple, Puzzle blue). */
  sailColor: number;
  /** The destination's landmark emoji, flown from the masthead. Colour alone is
   *  a smudge at ferry distance, and blue-vs-purple is the hardest pair in the
   *  palette for a kid to tell apart across water; the boat literally carrying
   *  the island's sign is what makes two ferries readable. */
  glyph: string;
}
...
export function createFerry(THREE: ThreeNS, reduceMotion: boolean, L: FerryLayout): TownFerry {
  ...
  // Candy-cane mast + a ribbon sail in the destination's colour.
  ...
  boat.add(mast);
  const sailMat = track(
    new THREE.MeshStandardMaterial({ color: L.sailColor, roughness: 0.6, side: THREE.DoubleSide }),
  );
  const sail = new THREE.Mesh(track(new THREE.PlaneGeometry(0.9, 0.9)), sailMat);
  sail.position.set(0.28, DECK_Y + 0.7, -0.2);
  sail.rotation.y = Math.PI / 2;
  boat.add(sail);
  // Masthead pennant: the destination's badge, camera-facing so it reads from
  // every angle.
  const pennant = emojiSprite(THREE, L.glyph, 0.7);
  texs.push(pennant.tex);
  mats.push(pennant.mat);
  pennant.sprite.position.set(0, DECK_Y + 1.55, -0.2);
  boat.add(pennant.sprite);
  ...
  const dockPx: Record<FerryStop, { x: number; y: number }> = { [MAINLAND_STOP]: A, [L.islandId]: B };
  // Resting heading: face along the crossing toward the island.
  const restLen = Math.hypot(B.x - A.x, B.y - A.y) || 1;
  const restH = { hx: (B.x - A.x) / restLen, hy: (B.y - A.y) / restLen };
  let state: State = { kind: 'docked', at: MAINLAND_STOP };
  ...
  // Initial resting pose at the mainland dock, facing the island.
  ...
  return {
    ...
    depart(dest: FerryStop): boolean {
      ...
      if (!(dest in dockPx)) return false; // not a stop this boat serves
      const from = state.at;
