...
export interface City3D {
  ...
  /** Walkable pier decks. `contains` is ORIENTED -- a turned deck's bounding
   *  rect includes open water, so the rect is a bound and this is the test. */
  pierDecks: Array<{ rect: RectPx; contains: (px: number, py: number) => boolean; slug: string }>;
...
export function createCity3D(THREE: ThreeNS, opts: CreateCityOpts): City3D {
  ...
  const pierDecks: Array<{ rect: RectPx; contains: (px: number, py: number) => boolean; slug: string }> = [];
  ...
  for (const region of REGIONS) {
    ...
    //   'waffle'  — Puzzle Island's golden waffle griddle
    ...
    type PadStyle = 'checker' | 'track' | 'podium' | 'waffle' | 'flat';
    ...
    //
    // The waffle's 0.72 is load-bearing: a 4×3 rect is 8.8×6.6u, so the
    // griddle is 6.34×4.75u (half-depth 2.38u), and boothOffsetsPx puts the
    // booth row at +2.71u with a 1.8u body — the shops stand fully on grass
    // FACING the griddle, a market square with the waffle as its plaza floor.
    // Above ~0.8 they sit half on the texture, which reads as a bug.
    const padFactor =
      padStyle === 'flat' ? 0.96 : padStyle === 'track' ? 0.88 : padStyle === 'waffle' ? 0.72 : 0.58;
    ...
    if (padStyle === 'track') {
    ...
    } else if (padStyle === 'waffle') {
      // Puzzle Island: a golden waffle griddle. Nine-by-nine grooves — nine,
      // not ten, because the waffle IS the sudoku grid — with three-by-three
      // piped-frosting dividers, so the island teaches the box rule underfoot
      // before a kid ever opens the game. One butter pat and one syrup drizzle,
      // off-centre, so it reads as breakfast rather than graph paper.
      const waffleTex = track(
        texs,
        makeCanvasTexture(
          THREE,
          (ctx, w) => {
            ctx.fillStyle = colorHex(WORLD.WAFER);
            ctx.fillRect(0, 0, w, w);
            // Fine grooves: the pockets.
            const n = 9;
            const s = w / n;
            ctx.strokeStyle = colorHex(WORLD.WAFFLE_GROOVE);
            ...
            // Heavy dividers: the 3×3 boxes, piped in frosting with a shadow
            // line just below so the piping has body.
            ctx.lineCap = 'round';
            for (const [color, width, dy] of [
              [colorHex(WORLD.FROSTING_PATH_EDGE), w * 0.028, w * 0.004],
              [colorHex(CAKE.FROSTING), w * 0.028, 0],
            ] as const) {
              ...
              for (let i = 1; i < 3; i++) {
                ...
                ctx.moveTo(p, w * 0.02);
                ctx.lineTo(p, w * 0.98);
                ctx.moveTo(w * 0.02, p + dy);
                ctx.lineTo(w * 0.98, p + dy);
                ctx.stroke();
              }
            }
            // Butter pat in one pocket.
            ctx.fillStyle = colorHex(CAKE.VANILLA);
            const pat = w * 0.07;
            ctx.beginPath();
            ctx.roundRect(s * 1.5 - pat / 2, s * 1.5 - pat / 2, pat, pat, pat * 0.2);
            ctx.fill();
            // Syrup drizzle across two pockets on the opposite diagonal.
            ctx.strokeStyle = colorHex(CAKE.STRAWBERRY_DEEP);
            ctx.lineWidth = w * 0.014;
            ctx.beginPath();
            ctx.moveTo(s * 6.6, s * 6.4);
            ctx.quadraticCurveTo(s * 7.3, s * 7.9, s * 8.3, s * 7.2);
            ctx.stroke();
          },
          512,
          512,
      ...
      padMat = track(mats, new THREE.MeshStandardMaterial({ map: waffleTex, roughness: 0.9 }));
    ...
    if (kidCupcake) {
      ...
      cherry.castShadow = true;
      hero.add(cherry);
    } else if (region.slug === 'puzzle-isle') {
      // The Sugar Cube: a 3×3×3 stack of glossy sugar cubes on the shared
      // frosting plinth, rows coloured strawberry / vanilla / mint (the brand
      // cake as a puzzle), slow-spinning, with ONE cube out of place — mid-
      // solve, not decorative. Grid-shaped like the waffle and stacked like the
      // tower puzzle, without being a jigsaw (the wrong verb for either game).
      // One geometry reused 27 times, three shared materials.
      const plinth = new THREE.Mesh(
        track(geos, new THREE.CylinderGeometry(0.72, 0.88, 0.5, 22)),
        track(mats, new THREE.MeshStandardMaterial({ color: 0xfff1d6, roughness: 0.6 })),
      );
      plinth.position.y = 0.25;
      plinth.castShadow = true;
      hero.add(plinth);
      const grout = new THREE.Mesh(
        track(geos, new THREE.CylinderGeometry(0.8, 0.8, 0.12, 22)),
        track(mats, frostingMat(THREE, 0xffffff)),
      );
      grout.position.y = 0.52;
      hero.add(grout);
      const cubeGeo = track(geos, new THREE.BoxGeometry(0.62, 0.62, 0.62));
      const rowMats = [
        track(mats, candyMat(THREE, CAKE.STRAWBERRY)),
        track(mats, candyMat(THREE, CAKE.VANILLA_DEEP)),
        track(mats, candyMat(THREE, CAKE.MINT)),
      ];
      const stack = new THREE.Group();
      stack.position.y = 0.58;
      const PITCH = 0.68;
      for (let row = 0; row < 3; row++) {
        for (let ix = -1; ix <= 1; ix++) {
          for (let iz = -1; iz <= 1; iz++) {
            const cube = new THREE.Mesh(cubeGeo, rowMats[row]);
            cube.position.set(ix * PITCH, 0.35 + row * PITCH, iz * PITCH);
            cube.castShadow = true;
            // The unsolved cube: top-middle, twisted and lifted a little.
            if (row === 2 && ix === 0 && iz === 0) {
              cube.rotation.y = 0.34;
      ...
      hero.add(stack);
      spinners.push(stack);
      const halo = glowSprite(THREE, WORLD.GLOW_WARM, 3.0, 0.28);
      track(texs, halo.tex);
      track(mats, halo.mat);
      halo.sprite.position.y = 1.6;
      hero.add(halo.sprite);
    } else {
    ...
    if (!kidCupcake) {
      ...
      track(texs, land.tex);
      track(mats, land.mat);
      // The sugar-cube stack tops out near 2.6u, so its badge floats higher.
      land.sprite.position.set(0, region.slug === 'puzzle-isle' ? 2.9 : 2.3, 0);
    ...
    if (pierCfg && region.games.length > 0) {
      ...
      pierDecks.push({ rect: pier.deckRect, contains: pier.containsPx, slug: region.slug });
