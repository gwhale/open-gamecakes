...
export interface InviteCodeRow {
  ...
  /** Optional: signup with this code adopts THIS family instead of creating
   *  one. Set deliberately at mint time. Null is the normal case — see
   *  migration 0052 for why a claim is granted rather than guessed. */
  claim_family_id: string | null;
}
...
export async function createInviteCode(args: {
  notes?: string;
  expiresAt?: Date;
  /** Grant this code the right to adopt an existing, unowned family. Only
   *  ever set by the admin script, and only on purpose. */
  ...
  const { error } = await sb
    ...
    .insert({
      ...
      claim_family_id: args.claimFamilyId ?? null,
    });
