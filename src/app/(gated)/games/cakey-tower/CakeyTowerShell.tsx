...
// Cakey Tower shell — GameLauncher → CakeyTower3DHost.
//
// A candy-tower demolition puzzle: solve a problem to earn a BITE, tap a mint
// candy to eat it, don't let a strawberry gummy fall off the plate. No timer —
// it's a lives-based puzzle, so the duration picker is hidden. "Play again"
// bumps `runId` (part of the host's React key) so the whole engine + WebGL
// context are disposed and rebuilt cleanly for a fresh tower.
//
// Name and glyph match the registry row ('Cakey Tower', 🍡). The launcher used
// to say "Cakey Towers 🍬" while the booth in town said "Cakey Tower 🍡", and
// the blurb warned about a cherry — the cherry sits on the GOOD candy.
...
export default function CakeyTowerShell({
  ...
  if (!settings) {
    return (
      <GameLauncher
        gameTitle="Cakey Tower"
        gameGlyph="🍡"
        gameDescription="Answer a question to earn a bite, then tap a mint candy 🍬 to eat it. Eat every mint to win — but if a strawberry gummy falls off the plate, you lose a heart ❤️."
  ...
  return (
    <CakeyTower3DHost
      key={runId}
      title="Cakey Tower"
      subtitle={kidName ? `${kidName}'s Tower` : 'Cakey Tower'}
      kidName={kidName}
