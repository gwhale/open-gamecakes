...
export default async function TownPage({
  ...
  if (isGuest(kidId)) {
    // EVERY land, not just the starters.
    //
    // The sandbox already waives every charge in the town -- rides, storm
    // clears, game unlocks, land reveals all come free for a guest (see
    // unlock-gate.ts, which says gating them "would make the demo unplayable
    // for the exact games we most want shown off"). Starting them on two
    // starter lands was the one place that rule was not applied, and it left
    // the demo walking the long way round to things it is meant to be showing.
    //
    // It also opens Caramel Cove, which is what makes the submarine appear and
    // the dive reachable -- see town/deep/page.tsx, which now lets guests in.
    const guestDiscovered = REGIONS.map((r) => r.slug);
    const guestSpawn = pickSpawn(null, getStarterRegions().map((r) => r.slug));
    ...
    return (
      <ThreeTownHost
        ...
        spawnX={guestSpawn.x}
        spawnY={guestSpawn.y}
        initialDiscovered={guestDiscovered}
        initialBalance={GUEST_BALANCE}
        seenStorySlugs={[]}
        tokenNotices={[]}
  ...
  // Should Cakey point at the submarine? Only if the kid can actually reach it
  // and has never been. Best-effort: any failure here just means no nudge,
  // which is the pre-existing behaviour rather than a broken page.
  const { data: deep } = await sb
    .from('kid_deep_state')
    .select('dives')
    .eq('kid_id', kid.id)
    .maybeSingle<{ dives: number }>();
  const nudgeSubmarine = discovered.includes(SUB_DOCK_REGION) && (deep?.dives ?? 0) === 0;
