...
//
//   22 games       registry.ts + registry.local.ts. COUNT BOTH: 0303 moved the
//                  family's own games out of upstream, so registry.ts alone is
//                  20 and undercounts gamecakes.org by two.
//   18 words-mode  `wordsMode: true` across both registries. Do NOT trust the
//                  "Set on the 16 games" comment in registry.ts — it is stale,
//                  and copying it instead of counting is how this page shipped
//                  the wrong number once already.
//   4,900 puzzles  src/components/games/chess-puzzles/library.json length
//   13 regions     town/regions.ts (11) + town/regions.local.ts (2)
//   16 playable    GUEST_MOUNTS keys in games/guest-mounts.ts - what /play
//                  offers with no account. Fewer than 22 on purpose: the
//                  family's own games are not upstream's to demo, and a
//                  game with no row in that table simply does not appear.
//   65 skills      select count(*) from skills — 0049 added word-meaning
//
...
export const metadata: Metadata = {
  ...
  description:
    'Open-source games for kids where the question is the gate. 22 games in a walkable 3D town across maths, reading and chess, pitched at what your child can actually do and aimable at what their class is covering this month. You host it, you control it, your family\'s data stays yours.',
    ...
    <main className="flex flex-1 flex-col">
      ...
      <header className="mx-auto w-full max-w-4xl px-6 pb-4 pt-16 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-zinc-600 dark:text-zinc-300">
          It&rsquo;s a walkable cake town with 22 games in it, across maths, reading
        ...
        <div className="mt-9 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat n="22" label="games" />
          <Stat n="13" label="town regions" />
          <Stat n="65" label="skills tracked" />
        ...
        <div className="mt-9 flex flex-wrap justify-center gap-3">
          <Link
            href="/play"
            className="font-display rounded-full bg-rose-500 px-7 py-3.5 text-lg font-bold text-white shadow-sm transition hover:bg-rose-600"
          >
            Play one now
          </Link>
          <a
            href="https://github.com/gwhale/open-gamecakes"
            className="font-display rounded-full bg-zinc-200 px-7 py-3.5 text-lg font-bold text-zinc-800 transition hover:bg-zinc-300 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700"
          >
            Get the code
          </a>
        </div>
        <p className="mt-4 text-sm text-zinc-500 dark:text-zinc-400">
          Sixteen of the games run in your browser right now. No account, no
          password, nothing saved. You don&rsquo;t have to take my word for any of
          this &mdash; go and hit a maths wall and see what happens.
        </p>
      </header>
      ...
      {/* games */}
      <Section id="games" eyebrow="The games" title="Twenty-two of them, and the maths is load-bearing">
        <p>
        ...
        <p>
          Eighteen of them carry a <strong>Maths / Words toggle</strong>. The same game
          ...
          there are 22 games here and not 44.
        </p>
        <p>
      ...
      <Section id="open" eyebrow="Open source" title="Run it for your own kids">
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <Link href="/play"
                className="font-display rounded-full bg-emerald-600 px-6 py-3 font-bold text-white shadow-sm transition hover:bg-emerald-700">
            Try it first
          </Link>
