...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  // Tier check inline rather than via requireParentModeOrJson, deliberately.
  // This route answers in TWO shapes — JSON for the in-page panel, a 303 back
  // to /parent/tokens for the no-JS form — and the shared guard only speaks
  // JSON. Routing a browser form POST through it would render raw JSON at the
  // parent instead of the page they were on. The auth logic above is
  // hand-rolled for the same reason; this follows it rather than fighting it.
  if (!can(family.tier, 'family:settings')) {
    return isJson
      ? Response.json({ ok: false, error: 'tier_denied' }, { status: 403 })
      : redirect303(`${TOKENS_PATH}?error=tier_denied`);
  }
