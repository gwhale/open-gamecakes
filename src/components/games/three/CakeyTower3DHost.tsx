...
//
//   pose a math/word challenge → on correct earn a BITE → tap a mint candy to
//   eat it → the tower settles → repeat. Don't let a strawberry gummy tumble off
//   the plate and splat (−1 life). WIN when every mint is eaten; LOSE when lives
//   run out. No timer: it is a lives-based puzzle, and the rules (rules.ts)
//   guarantee a round can always be finished, so nothing needs to end it early.
//   Reuses the shared challenge contract, buildSessionSummary, sounds, and the
//   same modal + game-over markup as Castle Crumble.
//
//   Everything the physics does AFTER the kid's tap — a gummy sliding off a
//   second later, a mint tumbling away — is said out loud in a toast. A life
//   that vanishes with no sentence attached is the single thing that made the
//   rules "make no sense" to a seven-year-old.
...
import { resolveTowerTuning, resolveTheme, type TowerSceneProps } from '@/lib/games/three/tower/types';
import { candiesEaten, starsForRun } from '@/lib/games/three/tower/rules';
import type { CakeyTowerEngine } from '@/lib/games/three/tower/engine';

export interface CakeyTowerHostProps {
  /** Defaults to the game's name — the arcade mounts this host from a table
   *  and must not be able to render a nameless canvas. */
  title?: string;
  subtitle?: string;
  kidName?: string;
  gameSlug: string;
  sceneProps: TowerSceneProps;
  /** Where "back" goes when no route-based override applies (the arcade passes
   *  /play). Same documented fallback PhaserGameHost honours; the pathname rule
   *  in useGameBackTarget still wins when it fires. */
  backHref?: string;
  backLabel?: string;
  attemptMeta?: {
...
const DEFAULT_TITLE = 'Cakey Tower';

type NoticeKind = 'splat' | 'mint-tumbled' | 'lucky' | 'gummy';

/** What the kid is told when the physics decides something. Kid voice: one
 *  short sentence, the thing that happened, then what it cost or gave.
 *  `count` is how many of the same thing happened in one go — a collapse can
 *  drop two gummies in the same second. */
function noticeText(kind: NoticeKind, count: number): string {
  switch (kind) {
    case 'splat':
      return count > 1
        ? `${count} gummies fell off the plate! −${count} ❤️`
        : 'A gummy fell off the plate! −1 ❤️';
    case 'mint-tumbled':
      return count > 1
        ? `${count} mints tumbled away — you don’t need them now 🍬`
        : 'A mint tumbled away — you don’t need it now 🍬';
    case 'lucky':
      return 'Lucky! The gobstopper was a mint 🍬';
    case 'gummy':
      return 'Uh-oh — it turned into a gummy! Keep it on the plate ❤️';
  }
}
...
export default function CakeyTower3DHost(props: CakeyTowerHostProps) {
  ...
  const backLabel = (fallback: string): string => backOverride?.label ?? props.backLabel ?? fallback;

  const [phase, setPhase] = useState<Phase>('ready');
  const [challenge, setChallenge] = useState<{ challenge: Challenge; reason: string } | null>(null);
  const [flashWrong, setFlashWrong] = useState(false);

  // HUD. No durationMin: the launcher hides the clock, so the value
  // getSessionDuration() returns is whatever some other game last picked. The
  // attempts API's drip falls back to its own cap when it is absent — same as
  // the other untimed games (chess puzzles, the candles).
  const [bites, setBites] = useState(0);
  const [lives, setLives] = useState<number | null>(null);
  const [goodLeft, setGoodLeft] = useState(0);
  const [won, setWon] = useState(false);
  const [stars, setStars] = useState(0);
  // True once the first bite has been earned — the how-to strip shows until then.
  const [firstBiteEarned, setFirstBiteEarned] = useState(false);
  // The current toast. `key` changes per notice so a repeat re-animates.
  //
  // A LOST HEART WINS. When a tower collapses, two gummies and six mints can
  // leave the plate inside a second; if each notice simply replaced the last,
  // the "−1 ❤️" was on screen for a few milliseconds and the kid only ever
  // read "a mint tumbled away" — seen in the headless run, lives 3 → 1 with
  // no heart toast visible. So while a splat notice is showing, nothing
  // lower replaces it; a second splat joins it as a count instead.
  const [notice, setNotice] = useState<{ kind: NoticeKind; count: number; key: number } | null>(null);
  const noticeRef = useRef<{ kind: NoticeKind; count: number } | null>(null);
  const noticeTimer = useRef<number | null>(null);
  const showNotice = useCallback((kind: NoticeKind) => {
    const cur = noticeRef.current;
    if (cur && cur.kind === 'splat' && kind !== 'splat') return;
    const next = cur && cur.kind === kind ? { kind, count: cur.count + 1 } : { kind, count: 1 };
    noticeRef.current = next;
    setNotice({ ...next, key: Date.now() });
    if (noticeTimer.current !== null) window.clearTimeout(noticeTimer.current);
    noticeTimer.current = window.setTimeout(() => { noticeRef.current = null; setNotice(null); }, NOTICE_MS);
  }, []);
  ...
  const endRound = useCallback((didWin: boolean) => {
    ...
    // goodTotal is every mint that counted; the kid ate the difference. The old
    // line printed goodTotal itself, so a loss said "12 candies eaten" with 9
    // still standing on the plate.
    const eaten = candiesEaten({ goodTotal: stats?.goodTotal ?? 0, goodLeft: stats?.goodLeft ?? 0 });
    ...
    const summary = buildSessionSummary({
      ...
      metaLines: [
        ...
        `🍬 ${eaten} ${eaten === 1 ? 'candy' : 'candies'} eaten`,
  ...
  useEffect(() => {
    ...
    // Anonymous play (no attemptMeta) — nothing to save. Mirrors the guard
    // PhaserGameHost has always had; the 3D hosts required the prop, which is
    // why the anonymous arcade could only ever offer Phaser games.
    ...
    fetch('/api/attempts', {
      ...
      body: JSON.stringify({
        ...
        // durationMin deliberately absent — untimed game; see the HUD comment.
      }),
    })
  ...
  }, [phase, sessionSummary, props.attemptMeta]);
  ...
  return (
    ...
    >
        ...
        <header className="flex w-full max-w-lg items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold">{props.subtitle ?? title}</h1>
            </div>
          </div>
      ...
      >
        ...
        {/* HUD badges: bites · lives · mints left. Each one says what it is —
            three bare numbers behind three emoji was a puzzle of its own.
            Left-clustered so they never sit under the fullscreen chrome
            (Map / sound / fullscreen, top-right). */}
        <div className="pointer-events-none absolute inset-x-0 top-0 flex flex-wrap items-center justify-start gap-2 p-3">
          <span className="font-display flex items-center gap-1.5 rounded-full bg-zinc-900/80 px-3 py-1.5 text-sm font-bold text-white shadow backdrop-blur-sm">
            <span aria-hidden>🍴</span>
            <span className="tabular-nums">{bites}</span>
            <span className="text-xs font-semibold text-white/85">{bites === 1 ? 'Bite' : 'Bites'}</span>
          </span>
          <span className={`font-display flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-bold text-white shadow ${lowLives ? 'bg-red-500/80' : 'bg-zinc-900/80 backdrop-blur-sm'}`}>
            <span aria-hidden>❤️</span>
            <span className="tabular-nums">{lives ?? '—'}</span>
            <span className="text-xs font-semibold text-white/85">{lives === 1 ? 'Life' : 'Lives'}</span>
          </span>
          <span className="font-display flex items-center gap-1.5 rounded-full bg-zinc-900/80 px-3 py-1.5 text-sm font-bold text-white shadow backdrop-blur-sm">
            <span aria-hidden>🍬</span>
            <span className="tabular-nums">{goodLeft}</span>
            <span className="text-xs font-semibold text-white/85">{goodLeft === 1 ? 'Mint left' : 'Mints left'}</span>
          </span>
        </div>

        {/* Toast — what the physics just did, in words. Reduced motion: the
            pop becomes a fade (see the keyframes below). */}
        <div className="pointer-events-none absolute inset-x-0 top-14 flex justify-center px-3" role="status" aria-live="polite">
          {notice ? (
            <div
              key={notice.key}
              className="font-display max-w-full rounded-full border-2 border-white/70 bg-amber-100 px-4 py-2 text-center text-sm font-bold text-amber-950 shadow-lg"
              style={{ animation: 'tower-toast 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) both' }}
            >
              {noticeText(notice.kind, notice.count)}
            </div>
          ) : null}
        </div>

        {/* Orbit-to-scout controls (↺/↻) — bottom corners, out of the way of the
            tower. They used to sit at mid-height, exactly where a candy near the
            edge of the tower is, so a tap meant to eat spun the camera instead. */}
        ...
        {/* Take-a-bite gate — between bites, the tower settles; tap to pose the
            next problem. Until the first bite is earned, the one-line rules sit
            above it: this is the whole game in a breath. */}
        {phase === 'ready' ? (
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-end gap-3 pb-8">
            {!firstBiteEarned ? (
              <div className="font-display rounded-full bg-zinc-900/75 px-4 py-2 text-center text-sm font-bold text-white shadow backdrop-blur-sm">
                Eat the mints 🍬 · keep the gummies on the plate ❤️
              </div>
            ) : null}
            <button
              type="button"
              onClick={() => poseChallenge(REASON_FIRST)}
              className="pointer-events-auto animate-bounce rounded-full bg-pink-500 px-7 py-3.5 text-lg font-extrabold text-white shadow-xl ring-4 ring-white/60 transition active:scale-95 motion-reduce:animate-none"
            >
              🍴 Take a bite!
            </button>
          </div>
        ...
        {/* Playing hint — a bite is loaded; tap a mint. */}
        {phase === 'playing' ? (
          <div className="pointer-events-none absolute inset-x-0 bottom-0 flex flex-col items-center gap-2 pb-8">
            <div className="font-display rounded-full bg-zinc-900/70 px-4 py-2 text-sm font-bold text-white shadow backdrop-blur-sm">
              🍬 Tap a mint candy (cherry on top) to eat it!
            </div>
          </div>
        ) : null}
        <style>{`
          @keyframes tower-toast { 0% { transform: translateY(-8px) scale(0.85); opacity: 0; } 100% { transform: translateY(0) scale(1); opacity: 1; } }
          @media (prefers-reduced-motion: reduce) { @keyframes tower-toast { 0% { opacity: 0; } 100% { opacity: 1; } } }
        `}</style>
      </div>

      {isFullscreen ? null : (
        <div className="mt-4 flex gap-3">
          <ChromeNavLink href={backHref} variant="dark" size="md">{backLabel('← Back to Map')}</ChromeNavLink>
        </div>
      )}
        ...
        <div role="status" aria-live="polite" className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm">
          ...
          >
            ...
            <div className="mt-2 flex w-full flex-col gap-3 sm:flex-row sm:justify-center">
              ...
              <ChromeNavLink href={backHref} variant="dark" size="lg">{backLabel('← Back home')}</ChromeNavLink>
            </div>
          </div>
          <style>{`
