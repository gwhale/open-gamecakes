...
import type { TownEngine, TownPositionPayload, TownMinimap, CakeyMoveInfo, FerryPrompt } from '@/lib/town/three/engine';
...
import {
  ChromeNavButton,
  ChromeNavLink,
  type ChromeNavVariant,
} from '@/components/ui/ChromeNavLink';
...
export interface ThreeTownHostProps {
  ...
  /** True when this kid owns Caramel Cove but has never dived. Turns on Cakey's
   *  pointer to the submarine — a feature nobody can find does not exist. */
  nudgeSubmarine?: boolean;
...
export default function ThreeTownHost(props: ThreeTownHostProps): React.ReactElement {
  ...
  // Standing near a docked Cakey Ferry → show the "Take the ferry" prompt for
  // THAT boat (there is one per ferry island; the payload names it).
  const [nearFerry, setNearFerry] = useState<FerryPrompt | null>(null);

  // Standing beside Sub-Cake One on Caramel Cove's jetty → show the "Dive"
  // prompt. There is no price and no confirm: the cove itself was the purchase,
  // and the ocean is what the cove is FOR.
  const [nearSub, setNearSub] = useState(false);
  ...
  // Board the submarine. Uses the SAME save-then-navigate contract as a game
  // booth, and for the same reason: /town/deep is a different world with its own
  // engine, and surfacing has to put the cupcake back on the jetty it left from
  // rather than at the kid's home land.
  const doDive = useCallback((): void => {
    const state = engineRef.current?.getState();
    if (state) postPosition(state);
    router.push('/town/deep');
  }, [postPosition, router]);
  ...
  const onConfirmUnlock = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // /api/town/discover answers a guest with a synthetic { balance: 0 }
      // because the sandbox has no wallet to report. Trusting it here wiped the
      // ephemeral pill back to zero on every reveal, which is most of why the
      // sandbox felt broke. The guest's balance lives in sessionStorage and is
      // the client's to keep.
      if (!props.isGuest) setBalance(data.balance);
  ...
  }, [modalRegion, modalPending, props.isGuest]);
    ...
    (async () => {
      ...
      const engine = engineMod.createTownEngine(
        ...
        },
        {
          ...
          onNearSub: (near) => setNearSub(near),
  ...
  return (
    ...
    >
      ...
      {/* Cakey Ferry — appears at a dock. The first crossing to an island costs
          🪙1 + the island, and discovers it; every later trip, and the trip
          home, is free. Which island is the boat's business: the engine says
          which one is in range. */}
      {nearFerry && !riding && !ridingVehicle && !ferrying && !cutsceneStory ? (
        (() => {
          const dest = findRegion(nearFerry.regionSlug);
          const known = nearFerry.returning || discoveredSlugs.has(nearFerry.regionSlug);
          ...
          const price = arrivalPrice(dest?.unlock_cost ?? 0, 'ferry');
          const tooBroke = !known && balance < price;
          return (
            <button
              type="button"
              onClick={() => {
              ...
              <span className="flex flex-col items-start leading-tight">
                ...
                <span className="flex items-center gap-1">
                  {known ? (
                    ...
                    <>
                      Need <SugarTokenIcon />{price} — play games!
                    </>
                  ) : (
                    <>
                      ▶ Ferry to {dest?.name ?? 'the island'} (<SugarTokenIcon />{price})
                    </>
                  )}
                </span>
              </span>
      ...
      {/* Sub-Cake One — appears at the end of Caramel Cove's jetty. No price
          and no confirmation dialog: the cove's own DEEP unlock cost already
          bought this, and a kid standing on a jetty next to a submarine has
          made their intention fairly clear. */}
      {nearSub && !riding && !ridingVehicle && !ferrying && !busing && !cutsceneStory ? (
        <button
          type="button"
          onClick={() => {
            playStart();
          ...
          className="candy-shell fixed fixed-bottom-safe left-1/2 z-30 -translate-x-1/2 flex items-center gap-3 rounded-full px-6 py-3 text-base font-display font-bold transition-[transform,box-shadow,filter] duration-100 ease-out active:scale-95"
          style={{ minHeight: 'var(--min-tap-target)', '--c-from': 'var(--travel-from)', '--c-to': 'var(--travel-to)', '--c-ink': 'var(--travel-ink)', '--c-glow': 'var(--travel-glow)' } as React.CSSProperties}
          aria-label="Dive in Sub-Cake One"
        >
          <span className="text-2xl" aria-hidden>🤿</span>
          <span className="flex flex-col items-start leading-tight">
            <span className="text-[11px] font-bold uppercase tracking-wider text-white/80">
      ...
      <CakeyOverlay
        ...
        nudgeSubmarine={props.nudgeSubmarine ?? false}
        onPlayStory={(slug) => {
...
/** Quick way into the collection, parked beside the wallet.
 *
 *  It sits next to the coins on purpose: a kid who has just been paid is the
 *  kid thinking about what they own, and the two counts answer the same
 *  question. Icon-only because the top strip is already four controls wide on
 *  a phone — hence the explicit ariaLabel, which the pill would otherwise
 *  derive from a bare emoji.
 *
 *  Deliberately NOT a count badge. The town host does not know how many items
 *  the kid holds, and plumbing that through every /town render to decorate a
 *  button would put a database read on the critical path of the world loading. */
function BackpackLink({ variant }: { variant: ChromeNavVariant }): React.ReactElement {
  return (
    <ChromeNavLink
      href="/kids/inventory"
      variant={variant}
      size="sm"
      ariaLabel="Your collection"
    >
    ...
    <div className="pointer-events-none fixed left-3 top-20 z-30 select-none">
      <div className="rounded-2xl border border-white/60 bg-white/70 p-2 shadow-lg backdrop-blur-sm dark:border-white/10 dark:bg-zinc-900/70">
        ...
        <svg width={svgW} height={svgH} viewBox={`${-W * 0.06} ${-H * 0.06} ${W * 1.12} ${H * 1.12}`}>
          ...
          {/* Dotted ferry routes across the open sea — how you get there. One
              per ferry island, in that island's colour (matching its sail). */}
          ...
          {/* The submarine dock. Its own marker, not a zone dot, because it is
              not a land — it is the way OUT of the island, like the ferry route
              above it. Drawn last-but-one so it sits over the zone dots.
              This exists because the feature was unfindable without it: the
              jetty is small, off one edge of a land whose sign says Caramel
              Cove, and nothing anywhere told a kid to go and look. */}
          {data.subDock ? (
            <g>
              <circle
                cx={data.subDock.nx}
              ...
              >
                <animate
                  attributeName="r"
                  values={`${W * 0.045};${W * 0.062};${W * 0.045}`}
                  dur="2.4s"
                  repeatCount="indefinite"
                />
              </circle>
              {/* A dot of deep water under it, so it reads as "down", not "another land". */}
              <circle cx={data.subDock.nx} cy={data.subDock.ny} r={W * 0.018} fill="#0b2a5e" />
            </g>
          ) : null}
